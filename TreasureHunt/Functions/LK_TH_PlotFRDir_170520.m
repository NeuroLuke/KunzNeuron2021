function f = LK_TH_PlotFRDir_170520(dt)
%
% LK_TH_PlotFRDir_170520 plots firing rate as a function of direction.
%
...
t5 = text(0.175, 0.15, ['max ', num2str(max(dt.FR), '%.2f')], ...
    'units', 'normalized', ...
...
% cave: flip x-axis left-right, because the arena is programmed such that
% yaw values of 0° point to the left
if dt.bReverseX == true
    set(gca, 'xdir', 'reverse');
end
% cave: flip z-axis up-and-down, because the arena is programmed such that
% negative yaw-values point to north
if dt.bReverseZ == true
...
maxFR = max(dt.FR); % maximum FR
hold on;
for iA = 1:numel(dt.direc.angleCenters)
    thisAngles  = transpose([...
        0, dt.direc.angleCenters(iA) - deg2rad(dt.direc.angularRes / 2):0.001:dt.direc.angleCenters(iA) + deg2rad(dt.direc.angularRes / 2), 0]);
...
% circular mean
circM               = circ_mean(dt.direc.angleCenters', dt.FR);
[circMx, circMy]    = pol2cart(circM, 1); % radius is set to 1

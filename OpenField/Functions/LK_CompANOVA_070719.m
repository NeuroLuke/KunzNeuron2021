function out = LK_CompANOVA_070719(dt)
%
% LK_CompANOVA_070719 computes an n-way ANOVA to estimate the influence of
% multiple predictors on neuronal firing rate.
% 
...
%
% The following predictors are exempted from the "minNumObsPerBin"
% criterion: Obj, Memor, dist2COM.
%
% Lukas Kunz, 2021
...
levelsOfInterest    = cell(size(dt.groupNames)); % e.g., 3 x 1; each cell entry contains the unique levels for each group (predictor)
...
for iGroup = 1:size(dt.group, 2)
    ...
    % CAVE: do not remove group levels for select predictors
    if strcmp(dt.groupNames{iGroup}, 'Obj') || strcmp(dt.groupNames{iGroup}, 'Memor') || strcmp(dt.groupNames{iGroup}, 'dist2COM')
...
% adjusted means for each factor of each predictor
m   = cell(numel(p), 1);
for ip = 1:size(m, 1)
...
out.m                   = m; % adjusted/estimated means

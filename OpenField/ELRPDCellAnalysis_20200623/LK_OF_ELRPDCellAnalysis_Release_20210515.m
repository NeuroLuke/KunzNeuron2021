...
paths.save      = strcat('E:\OpenField\ELRPDCellAnalysis_20200623\20200617_Dir', num2str(direc.angularRes), ...
...
exByNumSpikes   = []; % excluded by low number of spikes
...
for iSub = 1:size(subjects, 1)
    ...
    if exist(behLRPFile, 'file')
        ...
        outBehLRP  	= cell(size(locDir.LRPs, 1), 1); % e.g., size = 144 x 1
    ...
    for iWire = 1:size(wires, 1)
        ...
        for iClus = 1:max(ccb.cluster_class_behtime(:, 1))
            ...
            thisCluster = ccb.cluster_class_behtime(ccb.cluster_class_behtime(:, 1) == iClus, :); % cluster-class, microtime (msec), behavioral time (sec)
            ...
            if exist(resFile, 'file')
                ...
                % preallocate
                locdir_F        = nan(size(locDir.LRPs, 1), 1); % e.g., size = 144 x 1
                locdir_corrFR   = nan(size(locDir.LRPs, 1), numel(locDir.angleCenters)); % e.g., size = 144 x 12
                for iLRP = 1:size(locDir.LRPs, 1)
                    
                    % compute ANOVAN
                    dt              = [];
                    dt.FR           = FR(bMask4Analysis);
                    dt.group        = [outBeh.yawBin(bMask4Analysis), outBeh.xyBin(bMask4Analysis), outBehLRP{iLRP}.egoLocDirBin(bMask4Analysis)];
            ...
            if exist(resFile, 'file')
                ...
                % preallocate
                locdir_F_surro          = nan(size(locDir.LRPs, 1), param.numSurrogates); % e.g., size = 144 x 101
            ...
            %% allocentric direction tuning at each candidate LRP (plus surrounding area)
            ...
            if exist(resFile, 'file')
                ...
                % loop through LRPs
                dir_F_perLRP        = nan(size(locDir.LRPs, 1), 1); % e.g., size = 144 x 1
                dir_corrFR_perLRP   = nan(size(locDir.LRPs, 1), size(direc.angleCenters, 1)); % e.g., size = 144 x 12
                for iLRP = 1:size(locDir.LRPs, 1)
            ...
            locdir_Frank            = sum(locdir_F > locdir_F_surro, 2) ./ sum(~isnan(locdir_F_surro), 2); % e.g., size = 144 x 1
            ...
            print(f, strcat(paths.save, subjects{iSub}, '_', wires(iWire).name, '_Clus', num2str(iClus), '_ELRPD_20210408'), '-dtiff', '-r450');
            close(f);
...
% direction and place-like cells
PxD     = load('E:\OpenField\PlaceDirAnalysis_030620\20201119_Dir30_Loc10x10\bNaviIfAbove0.001AndbRotation_Smooth2\results.mat', ...
    'allRes');
...
% report
fprintf('\nNumber of direction cells from PxD analysis: %d (P = %.3f).\n', sum(PxD_bDirCell), ...
    myBinomTest(sum(PxD_bDirCell), numel(PxD_bDirCell), 0.05));
fprintf('Number of place cells from PxD analysis: %d (P = %.3f).\n', sum(PxD_bPlaceCell), ...
    myBinomTest(sum(PxD_bPlaceCell), numel(PxD_bPlaceCell), 0.05));

%% ELRPD cells

% identify ELRPD cells and report
bELRPDCell      = cell2mat({r.allRes.locdir_clusRank}') > 0.95; % >0.95 corresponds to P<0.05
...
LK_EvalELRPDCells_20210409(dt);

%% overlap between different cell types

fprintf('\nOverlap of cells.\n');

% ELRPD cells
fprintf('%d cells are both direction and ELRPD cells.\n', sum(PxD_bDirCell & bELRPDCell));
fprintf('%d cells are both place and ELRPD cells.\n', sum(PxD_bPlaceCell & bELRPDCell));
fprintf('%d cells are both direction, and place, and ELRPD cells.\n', sum(PxD_bDirCell & PxD_bPlaceCell & bELRPDCell));

%% region-wise distribution of ELRPD cells

fprintf('\nEvaluation of region-wise distribution of ELRPD cells.\n');
...
set(f, 'PaperPositionMode', 'auto');
print(f, strcat(r.paths.save, 'ELRPDCell_PerRegion_20210408'), '-dtiff', '-r300');

%==========================================================================
% This script analyzes egocentric bearing cells (= ELRPD cells) using
% random data to estimate the empirical alpha level.
%
% Lukas Kunz, 2021
%==========================================================================
...
paths.save      = strcat('E:\OpenField\ELRPDCellAnalysis_20200623\20200921_randData', num2str(param.myRNG), '_Dir', num2str(direc.angularRes), ...
...
for iSub = 1:size(subjects, 1)
    ...
    if exist(behLRPFile, 'file')
        ...
        parfor iLRP = 1:size(locDir.LRPs, 1)
            ...
            % update the information regarding the local reference point
            dt                  = [];
            dt.paths            = paths;
            dt.subject          = subjects{iSub};
            dt.direc            = direc;
    ...
    for iWire = 1:size(wires, 1)
        ...
        for iClus = 1:max(ccb.cluster_class_behtime(:, 1))
            ...
            % CAVE: produce surrogate data here instead of using the real
            % data
            FR  = circshift(FR, datasample(1:length(FR) - 1, 1));
            ...
            % rate by egocentric direction towards this point
            ...
            if exist(resFile, 'file')
                ...
                % loop through LRPs
                dir_F_perLRP        = nan(size(locDir.LRPs, 1), 1); % e.g., 144 x 1
                dir_corrFR_perLRP   = nan(size(locDir.LRPs, 1), size(direc.angleCenters, 1)); % e.g., 144 x 12
                for iLRP = 1:size(locDir.LRPs, 1)
